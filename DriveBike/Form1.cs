...
namespace DriveBike
{
    public partial class Form1 : Form
    {
        ...
        private void UpdateAllImages(string url)
        {
            ...
            {
                otv = nethouse.getRequest("https://bike18.ru" + razdels[i].ToString() + "?page=all");
                MatchCollection tovars = new Regex("(?<=<div class=\"product-item__content\"><a href=\").*?(?=\")").Matches(otv);
                foreach (Match s in tovars)
                {
